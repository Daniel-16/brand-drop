  ...
  return (
    <motion.button
      ...
      style={{
        ...
        color: copied ? "#4CAF7D" : "var(--text-muted)",
        cursor: "pointer",
        fontSize: 12,
        display: "flex",
        alignItems: "center",
...
export default function Step4Results({
  ...
  const [activeTab, setActiveTab] = useState(
    availableTabs[0]?.value || "instagram",
  );
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (imageBase64) {
        ...
        folder.file("campaign_image.png", imgData, { base64: true });
      }

      const blob = await zip.generateAsync({ type: "blob" });
      saveAs(
        blob,
        `BrandDrop_${businessName.replace(/\s+/g, "_")}_Campaign.zip`,
      );
    } finally {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 36 }}>
        ...
        >
          Your <span className="text-gold-gradient">{businessName}</span>{" "}
          campaign kit
        </h1>
            ...
            {availableTabs.map((tab) => {
              ...
              return (
                ...
                >
                  <Icon
                    size={14}
                    style={{ color: active ? tab.color : "inherit" }}
                  />
                  {tab.label}
                </motion.button>
              );
            ...
            >
              ...
              {activeTab === "instagram" && content.instagram && (
                <>
                  ...
                  <ContentBlock
                    label="Hashtags"
                    value={content.instagram.hashtags.join(" ")}
                  />
                </>
              )}
              {activeTab === "facebook" && content.facebook && (
                <>
                  ...
                  <ContentBlock
                    label="Caption"
                    value={content.facebook.caption}
                  />
                  <ContentBlock
                    label="Hashtags"
                    value={content.facebook.hashtags.join(" ")}
                  />
                </>
              )}
              {activeTab === "whatsapp" && content.whatsapp && (
                <>
                  <ContentBlock
                    ...
                    value={content.whatsapp.statusCaption}
                  />
                  <ContentBlock
                    label="Broadcast Message"
                    value={content.whatsapp.broadcastMessage}
                  />
                </>
              )}
            </motion.div>
